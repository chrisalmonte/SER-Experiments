...
TEST_NAME = 'over_CremaD'
model_manager = ModelManager('output\models\wavlemo\class', new_run=False)
...
df_test = pd.read_csv("output/processing/custom_labels/crema-d/cremad_labels.csv")
df_test = df_test[df_test["EmoClass"].isin(['N', 'H', 'S', 'A', 'F', 'D'])]
    ...
    with torch.no_grad():
        for inputs, masks, targets in tqdm(dataloader, total=len(dataloader)):
            ...
            with torch.amp.autocast('cuda'):
                ...
                #set surprise logits to minus infinity to ignore them in metrics
                if num_classes == 7:
                    logits[:, 6] = float('-inf')
