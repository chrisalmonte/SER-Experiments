...
# -------------------------- Create data loaders --------------------------
#Train set
dataset_train = cAudiotools.VADSubdirAudioDatasetGender(
...
dataset_dev = cAudiotools.VADSubdirAudioDatasetGender(
...
inputs, masks, targets, genders = sample_batch
...
log.log_message(f"Genders Shape: {genders.shape}")
...
def train_loop(dataloader, model, loss_fn, optimizer, metrics_dict=None, pinned_memory=False):
    ...
    for batch, (inputs, masks, targets, genders) in tqdm(enumerate(dataloader), total=len(dataloader)):
